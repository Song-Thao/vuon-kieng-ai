  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white p-4 max-w-4xl mx-auto">
      <div className="flex justify-between items-center mb-6">
        ...
        <a href="/marketplace/dang-ban" className="bg-green-600 hover:bg-green-500 rounded-lg px-4 py-2 text-sm font-semibold">
          + Đăng bán
        </a>
      </div>

      <input
        className="w-full bg-gray-800 rounded-lg p-3 mb-6 text-white placeholder-gray-500"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {filtered.map(item => (
            <div key={item.id} className="bg-gray-800 rounded-xl overflow-hidden hover:ring-1 hover:ring-green-500 transition">
              {item.hinh_anh && (
                <img src={item.hinh_anh} alt={item.ten_cay} className="w-full h-48 object-cover" />
              )}
              <div className="p-4">
                ...
                <div className="flex justify-between items-center">
                  <div className="text-sm text-gray-500">
                    {item.vi_tri && <span>📍 {item.vi_tri}</span>}
                  </div>
                  <div className="flex gap-2">
                    ...
                    {item.sdt && (
                      <a href={`tel:${item.sdt}`}
                        className="bg-green-700 hover:bg-green-600 rounded px-3 py-1 text-xs">
                        Gọi
                      </a>
                    )}
                  </div>
