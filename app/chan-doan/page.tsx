...
export default function ChanDoan() {
  ...
  const fields = [
    { key: 'ten_cay', label: 'Tên cây', icon: '🌿', ph: 'VD: Mai vàng, Bonsai sanh, Kim tiền...' },
    { key: 'moi_truong', label: 'Môi trường trồng', icon: '🪴', ph: 'VD: Trên chậu, dưới đất, mới vô chậu...' },
    { key: 'vung', label: 'Vùng khí hậu', icon: '🌍', ph: 'VD: Miền Nam - Cà Mau, Miền Trung...' },
  ...
  return (
    <>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Playfair+Display:wght@500;700&family=Be+Vietnam+Pro:wght@300;400;500;600&display=swap');
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .cd-wrap {
          ...
          background-image: 
            radial-gradient(ellipse at 20% 50%, rgba(45,90,50,0.3) 0%, transparent 60%),
            radial-gradient(ellipse at 80% 20%, rgba(26,58,30,0.4) 0%, transparent 50%);
        ...
        .cd-hero {
          background: linear-gradient(180deg, rgba(13,31,15,0) 0%, var(--forest) 100%),
            linear-gradient(135deg, var(--moss) 0%, var(--leaf) 50%, var(--moss) 100%);
        ...
        .cd-hero h1 span { color: var(--mint); }
        ...
        .cd-body { max-width: 600px; margin: 0 auto; padding: 0 16px; }
        ...
        .cd-card-title::after {
          ...
          flex: 1;
          height: 1px;
          background: linear-gradient(90deg, rgba(127,184,138,0.3), transparent);
        }
        ...
        .cd-field textarea::placeholder { color: rgba(184,212,190,0.35); font-size: 13px; }

        .cd-field input:focus,
        .cd-field textarea:focus {
          border-color: rgba(127,184,138,0.5);
          background: rgba(13,31,15,0.8);
          box-shadow: 0 0 0 3px rgba(127,184,138,0.08);
        }

        .cd-field textarea { height: 90px; resize: vertical; }

        .cd-upload-area {
          border: 2px dashed rgba(127,184,138,0.25);
          border-radius: 16px;
          overflow: hidden;
        ...
        .cd-upload-text { color: var(--dew); font-size: 14px; margin-bottom: 4px; }

        .cd-upload-sub { color: rgba(184,212,190,0.5); font-size: 12px; }

        .cd-upload-area input[type=file] {
          position: absolute;
          inset: 0;
          opacity: 0;
          cursor: pointer;
        ...
        .cd-preview-change {
          background: rgba(13,31,15,0.8);
          border: 1px solid rgba(127,184,138,0.4);
          color: var(--mint);
          padding: 8px 20px;
          border-radius: 20px;
          font-size: 13px;
        ...
        .cd-btn:hover:not(:disabled)::before { opacity: 1; }
        .cd-btn:hover:not(:disabled) { transform: translateY(-1px); box-shadow: 0 8px 24px rgba(45,90,50,0.4); }
        .cd-btn:disabled { background: rgba(74,124,89,0.3); cursor: not-allowed; color: rgba(255,255,255,0.4); }
        ...
        .cd-result { animation: fadeUp 0.4s ease; }
        ...
        .cd-result-header {
          ...
          margin-bottom: 20px;
          padding-bottom: 16px;
          border-bottom: 1px solid rgba(127,184,138,0.15);
        }
        ...
        .cd-info-box {
          ...
          border: 1px solid rgba(127,184,138,0.12);
          border-radius: 14px;
          padding: 14px;
        }

        .cd-info-label { font-size: 10px; text-transform: uppercase; letter-spacing: 1px; color: var(--sage); margin-bottom: 6px; }

        .cd-info-value { font-size: 15px; font-weight: 600; color: var(--cream); }
        ...
        .cd-section {
          background: rgba(13,31,15,0.4);
          border: 1px solid rgba(127,184,138,0.1);
          border-radius: 14px;
          padding: 16px;
          margin-bottom: 10px;
        }

        .cd-section-title { font-size: 11px; text-transform: uppercase; letter-spacing: 1px; color: var(--sage); margin-bottom: 10px; }

        .cd-cause-main { color: var(--amber); font-weight: 600; font-size: 15px; margin-bottom: 6px; }
        ...
        .cd-step-text { font-size: 14px; color: var(--dew); padding-top: 3px; line-height: 1.5; }

        .cd-care-row { display: flex; gap-y: 6px; margin-bottom: 8px; font-size: 13px; }
        .cd-care-icon { width: 24px; text-align: center; margin-right: 8px; }
        .cd-care-label { color: var(--sage); margin-right: 6px; font-size: 12px; }
        .cd-care-val { color: var(--dew); }

        .cd-shopee-btn {
          display: block;
          background: linear-gradient(135deg, #ee4d2d, #d03b1f);
          border-radius: 10px;
          padding: 12px 16px;
          text-align: center;
          color: white;
        ...
        <div className="cd-body">
          ...
          <div className="cd-card">
            ...
            <div className="cd-field">
              ...
              <textarea
                placeholder="VD: Tưới mỗi sáng 1 lít, để ngoài trời nắng cả ngày, mới bón NPK 1 tuần trước, lá vàng từ rìa vào..."
                value={info.tinh_trang}
                onChange={e => setInfo({...info, tinh_trang: e.target.value})}
              />
            </div>
          </div>
              ...
              {image ? (
                <>
                  ...
                  <div className="cd-preview-overlay">
                    <span className="cd-preview-change">🔄 Đổi ảnh</span>
                  </div>
                </>
              ) : (
            ...
            <div className="cd-card cd-result">
              ...
              <div className="cd-grid">
                ...
                <div className="cd-info-box">
                  ...
                  <div className="cd-badge">
                    {result.muc_do === 'nang' ? '🔴 Nặng' : result.muc_do === 'trung_binh' ? '🟡 Trung bình' : '🟢 Nhẹ'}
                  </div>
                </div>
              </div>
              ...
              {result.mo_ta_benh && (
                <div className="cd-section">
                  <div className="cd-section-title">Mô tả triệu chứng</div>
                  <div style={{fontSize: 14, color: 'var(--dew)', lineHeight: 1.6}}>{result.mo_ta_benh}</div>
                </div>
              )}
              ...
              {result.nguyen_nhan_chinh && (
                <div className="cd-section">
                  ...
                  {result.nguyen_nhan_phu?.map((n: string, i: number) => (
                    <div className="cd-cause-sub" key={i}>• {n}</div>
                  ))}
                </div>
              )}
              ...
              {result.lich_cham_soc && (
                <div className="cd-section">
                  ...
                  {[
                    { icon: '💧', label: 'Tưới nước', val: result.lich_cham_soc.tuoi_nuoc },
                    { icon: '🌱', label: 'Bón phân', val: result.lich_cham_soc.bon_phan },
                    { icon: '☀️', label: 'Ánh sáng', val: result.lich_cham_soc.anh_sang },
                  ].map((r, i) => (
                    <div className="cd-care-row" key={i}>
                      <span className="cd-care-icon">{r.icon}</span>
                      <span className="cd-care-label">{r.label}:</span>
                      <span className="cd-care-val">{r.val}</span>
                    </div>
                  ))}
                </div>
              )}
              ...
              {result.san_pham_can?.length > 0 && (
                <div className="cd-section">
                  ...
                  {result.san_pham_can.map((s: string, i: number) => (
                    <a key={i} href={`https://shopee.vn/search?keyword=${encodeURIComponent(s)}`}
                      target="_blank" className="cd-shopee-btn">
                      🛒 {s} → Tìm trên Shopee
                    </a>
                  ))}
                </div>
